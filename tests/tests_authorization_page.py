...
class TestAuthorizationPage:
    ...
    def test003_valid_authorization(self, auth, username):
        ...
        auth.click_element(auth.logout_btn)
        assert auth.find_element(auth.page_logo).text == auth.auth_title, 'Логотип открытой страницы не соответствует эталону, либо перешли по другой ссылке'
    ...
    # Негативный тест авторизации по пустому полю ввода телефона и валидному паролю, и пустому полю пароля. Появляется сообщение об ошибке.
    ...
    # Негативный тест авторизации по пустому полю ввода лицевого счета и валидному паролю. Появляется сообщение об ошибке.
    ...
    # Проверка авторизации по валидному телефону и паролю при вводе телефона в таб "Лицевой счет"
    @pytest.mark.fail_if_captcha
    ...
    # Проверка перехода на страницу авторизации через соц. сеть Вконтакте
    ...
    # Проверка перехода на страницу авторизации через соц. сеть Однокласники
    ...
    # Проверка перехода на страницу авторизации через сервис клиент Yandex ID
